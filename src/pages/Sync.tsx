...
export default function Sync() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setCode(newCode);
      setMessage({ kind: 'ok', text: 'Created a new sync code and pushed everything currently saved on this device.' });
    } catch (err) {
  ...
  async function handlePush() {
    ...
    try {
      ...
      setMessage({ kind: 'ok', text: 'Pushed everything currently saved on this device to the sync code.' });
    } catch (err) {
      setMessage({ kind: 'error', text: (err as Error).message });
    } finally {
      setBusy(null);
  ...
  return (
    <div>
      ...
      <p className="mb-6 max-w-3xl text-sm text-stone-500">
        Move your characters between browsers and devices with a shareable code — no account or sign-in required.
      </p>

      <div className="card mb-6 p-5">
        <h2 className="section-title">How this works</h2>
        <ul className="ml-4 list-disc space-y-1 text-sm text-stone-500">
          <li>This uses a free, public, anonymous JSON storage service (jsonblob.com) as a simple shared save slot.</li>
          <li>Anyone with your code can read or overwrite that data — treat it like an unlisted link, not a password.</li>
          <li>There's no encryption and no account behind it. For anything sensitive, or if this service is ever down, Export/Import JSON on the Characters page always works as a reliable manual alternative.</li>
        ...
        {code ? (
          ...
          <div className="mb-4">
            ...
            <button className="btn-primary" onClick={handleCreate} disabled={busy !== null}>
              {busy === 'create' ? 'Creating…' : 'Create Sync Code'}
            </button>
          </div>
        )}
